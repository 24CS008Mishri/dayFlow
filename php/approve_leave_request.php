<?php
// approve_leave_request.php - Approve a leave request
header('Content-Type: application/json');

include 'config.php';
session_start();

// Check authentication - only admin/hr can approve
...
if ($stmt->execute()) {
    ...
    if ($selectStmt) {
        ...
        if ($activityStmt) {
            $activityType = 'Leave Request Approved';
            $description = 'Approved leave request from ' . $row['start_date'] . ' to ' . $row['end_date'];
